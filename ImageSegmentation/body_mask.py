...
def apply_body_mask_and_bound(array: np.ndarray, masking_value: int =-1024, \
    ...
    if apply_bound:
        ...
        array = array[z_max:z_min+1, y_max: y_min, x_max: x_min]
    else:
        mask = np.ones(array.shape)
        mask[z_max:z_min + 1, y_max: y_min, x_max: x_min] = 1
        array = array * mask
    return array
...
if __name__=='__main__':
    ...
    x=apply_body_mask_and_bound(array,keep_original_size=True)
    # x.reshape(x.shape[1],x.shape[2],1)
    # plt.imshow(x)
    # plt.show()
